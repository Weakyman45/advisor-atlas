...
function defaultProgress(professor) {
  return {
    ...
    nextActionDue: "", notes: "", lastUpdated: "",
  };
}
...
function upsertStatement(db, id, progress) {
  return db.prepare(`INSERT INTO professor_progress (
    ...
    application_planned, application_submitted, next_action, next_action_due, notes, last_updated
  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ON CONFLICT(professor_id) DO UPDATE SET
    ...
    next_action_due = excluded.next_action_due, notes = excluded.notes, last_updated = excluded.last_updated`).bind(
...
async function saveOne(request, env) {
  ...
  try { await ensureSchema(env.DB); await upsertStatement(env.DB, body.id, progress).run(); return json({ ok: true, progress }); }
